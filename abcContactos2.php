	...
	if (isset($_SESSION["usu"]) && !empty($_SESSION["usu"])){
		...
		if (isset($_POST["txtClave"]) && !empty($_POST["txtClave"]) &&
			...
			if ($sOpe == 'a'){
				...
				$bLlaveEditable = true;
				$sNomBoton ="Agregar";
                                                                $mensajeaux='Deseas'.$sNomBoton.'al contacto:'.$oUsurCont->getNombre().'?';
			}
			else if ($sOpe == 'm'){
				...
				$sNomBoton ="Modificar";
                            ...
                            $bCampoEditable = false ; //la llave no es editable por omisión
				$sNomBoton ="Borrar";
                                     $mensajeaux='¿Deseas '.$sNomBoton.' al contacto: '.$oUsurCont->getNombre().' ? ';
                        }
...
?>
		<section>
			<form name="abcPH" action="conABCdos.php" method="post">
				...
				<br/>
                                                                 id_visualizador
                                                                 <input type="text" name="txtVisualizador" 
					<?php echo ($bCampoEditable==true?'':' disabled ');?>
                                                                        value="<?php echo $oUsurCont->getIdPersonal();?>"/>
				<br/>
				...
				<input type ="submit" value="<?php echo $sNomBoton;?>" 
                                                                onClick="return mostrarVentanaEmergente(event, '<?php echo $mensajeaux;?>', 'Sí', 'No', 'tabcontactosdos.php', 'img/pregunta.gif');">
				<input type="submit" name="Submit" value="Cancelar" 
				 onClick="abcPH.action='tabcontactosdos.php';">
			</form>
		</section>
<?php
